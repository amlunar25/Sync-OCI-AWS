...
def lambda_handler(event, context):
    ...
    s3_folder_prefix = 'approach-2/'
    
    try:
        # Extract new file names from the event payload
        body = json.loads(event['body'])
        new_file = body['resourceName']
        
        # Call method to sync files
        res = sync_new_files_from_oci_to_s3(oci_bucket_name, s3_bucket_name, s3_folder_prefix, new_file)
